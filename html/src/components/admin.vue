...
<div>
    ...
    <div id="addCase" >
    <img src="../assets/addIcon.png" @click="addCase = !addCase">  新增案例<br>
   ...
   <div id="caseSearch">
        <input  v-model="countryFilter" type="text" class="right border"  placeholder="國家">
   </div>
       <div id="caseView">
      <table>
       <tr>
          ...
          <th>死亡人數</th>
          <th>時間</th>
       <th> &ensp;<img @click='editCaseShow = !editCaseShow'  src="../assets/editIcon.png" width="20" height="20"/>&ensp;
      &ensp;<img  @click='deleteCaseShow = !deleteCaseShow' src="../assets/delete.png" width="20" height="20"/>&ensp;</th>

        </tr>
          <tr :key="caseData" v-for="(caseData,index) in filteredCaseRows">
                <th>{{caseData.countryName}}</th>
                <th><input v-if='index==editItem' :placeholder="caseData.recover" type="number" v-model="editCaseR" >
                <a v-else >{{caseData.recover}}</a></th>
                <th><input v-if='index==editItem' :placeholder="caseData.sick" type="number" v-model="editCaseS" >
                <a v-else >{{caseData.sick}}</a></th>
                <th><input v-if='index==editItem' :placeholder="caseData.dead" type="number" v-model="editCaseD">
                <a v-else >{{caseData.dead}}</a></th>
                <th>{{caseData.time}}</th>
                <th>
                  <img v-if='index==editItem' src="../assets/finish.png" @click="editCaseFinish(index)">
                  <img v-if="editCaseShow" @click='editCase(index)' src="../assets/editIcon.png" width="20" height="20"/>
                 <img v-if="deleteCaseShow" @click='deleteCase(index)' src="../assets/cut.png" width="20" height="20"/>
               </th>
            </tr>
            </table>
    </div>
    ...
    <div id="userView">
     <table>
       <tr>
          ...
          <th>姓名</th>
          <th>身份</th>
           <th>&ensp;<img @click='deleteUserShow = !deleteUserShow' src="../assets/delete.png" width="20" height="20"/>&ensp;</th>
        </tr>
        <tr :key="userData" v-for="(userData,index) in filteredUserRows">
                <input v-if='userData.email==editUserEmail' :placeholder="userData.email" v-model="newEmail" type="text" @keypress.enter='leftEditEmail(index)'>
                <th v-else @click='intoEditEmail(userData.email)' >{{userData.email}}</th>
                <input v-if='index===editUserName' :placeholder="userData.name" v-model="newName" type="text" @keypress.enter='leftEditName(index)'>
                <th v-else @click="intoEditName(index)" > {{userData.name}}</th>
                <th>
                  <!--不能v-model 因為每列不一樣 and onchange不會觸發-->
                <select @onchange="editUserIdentity()">
                  <option selected :value="userData.userIdentity">{{userData.userIdentity}}</option>
                  <option v-if="userData.userIdentity!='user'" value="user">user</option>
                  <option  v-if="userData.userIdentity=='user'" value="admin">admin</option>
                </select>
                </th>
                <th>
                <img v-if="deleteUserShow" @click='deleteUser(userData.email,index)' src="../assets/cut.png" width="20" height="20"/>
                </th>
      </tr>
      </table>
      </div>
    ...
    export default{
        ...
        methods:{
            ...
            leftEditEmail(userIndex){
                this.editUserEmail='';
                if(this.newEmail!=''){
                  this.updateUserEmail(this.newEmail,this.allUserData[userIndex].email,userIndex);
                }
                /* else{
                  alert("不要亂點^_^")
                }*/
            ...
            leftEditName(userIndex){
                this.editUserName='';
                if(this.newName!=''){
                  this.updateUserName(this.newName,this.allUserData[userIndex].email,userIndex);
                }
               /* else{
                  alert("不要亂點^_^")
                }*/
            ...
            addCaseData(){
                if (this.caseTime==''||this.caseCountry==''||this.caseRecover==''||this.caseDead==''||this.caseSick==''){
                    alert("請填入新增資料，無請補零");
                }
                else{
                    let addCaseData={
            ...
            getAllUserData(){
                this.$http.get('/getAllUserData').then((res)=>{
                  if(res.data=="error"){
                    alert("抓取資料庫資料錯誤")
                  }
                  else{
            ...
            getAllCaseData(){
                this.$http.get('/getAllCaseData').then((res)=>{
                  if(res.data=="error"){
                    alert("抓取資料庫資料錯誤")
                  }
                  else{
            ...
            updateUserEmail(newEmail,userEmail,index){
               let postData={
                ...
                this.$http.post('/updateUserEmail',JSON.stringify(postData)).then((res)=>{
                        if(res.body==='success'){
            ...
            updateUserName(newName,userEmail,index){
               let postData={
                ...
                this.$http.post('/updateUserName',JSON.stringify(postData)).then((res)=>{
                        if(res.body==='success'){
            ...
            deleteUser(userEmail,index){
              ...
              let postData={
                ...
                this.$http.post('/deleteUser',JSON.stringify(postData)).then((res)=>{
                        if(res.body==='success'){
            ...
            if (this.editCaseR==''||this.editCaseS==''||this.editCaseD==''){
                    alert("請不要留下空白欄位");
                }
                else{
            let postData={
        ...
        computed: {
            selectAll: {
                get() {
                    return this.userComputers ? this.checkComputers.length === this.userComputers.length : false
                },
                set(value) {
                    let selected = []
                    if (value) {
          ...
          filteredUserRows: function(){
            ...
            return (this.emailFilter==''&& this.nameFilter==''&&this.userIdentityFilter=='') ? this.allUserData:
            this.allUserData.filter(function(d){ return d.name.indexOf(nameFilter) > -1 && d.email.indexOf(emailFilter) > -1 && d.userIdentity.indexOf(userIdentityFilter) > -1 });
          },
          filteredCaseRows:function(){
            var countryFilter=this.countryFilter.toLowerCase();
             return (this.countryFilter=='') ? this.allCaseData:
            this.allCaseData.filter(function(d){ return d.countryName.toLowerCase().indexOf(countryFilter) > -1});
          }
        }
    }
</script>
